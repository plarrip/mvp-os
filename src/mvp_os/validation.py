...
def validate_state(data, check_gate_requirements=True):
    ...
    for k in sorted(REQUIRED-set(data)): errors.append(f"missing top-level field: {k}")
    ...
    hids={x.get("id") for x in data.get("hypotheses",[]) if isinstance(x,dict)}
    for i,e in enumerate(data.get("experiments",[])):
